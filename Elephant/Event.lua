...
local function HandleMessage(prat_msg, event, ...)
  if not Elephant.db.profile.prat and prat_msg then return end
  if not Elephant.db.profile.events[event] then return end
  ...
  if event == "CHAT_MSG_CHANNEL" or event == "CHAT_MSG_CHANNEL_NOTICE" then
    ...
    if cIndex == nil then cIndex = channelName end

    if Elephant:IsFiltered(cIndex) then return end

    if event == "CHAT_MSG_CHANNEL" then
      if not Elephant.dbpc.char.logs[cIndex].enabled then return end
    ...
    if event == "CHAT_MSG_CHANNEL_NOTICE" then
      ...
      if message == "YOU_LEFT" then
        if not Elephant.dbpc.char.logs[cIndex] then return end
        if not Elephant.dbpc.char.logs[cIndex].enabled then return end
