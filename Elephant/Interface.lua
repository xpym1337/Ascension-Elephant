...
local function AddColorStrings(msg, r, g, b)
  if r and g and b then
    ...
    if string.sub(msg, 1, 2) == "|r" then
      ...
      msg = "|c" .. Elephant:MakeTextHexColor(r, g, b) .. msg
    end
...
--[[
Cleans up some meta-types that won't be
used for copying, typically player links.

Item links are not removed since they may
be useful for BBCode output.
]]
local function CleanupMetaTypes(msg)
  msg = string.gsub(msg, "|Hplayer:.-|h(%[.-])|h", "%1")
  msg = string.gsub(msg, "|Hplayer:.-|h(%[|r|c%x%x%x%x%x%x%x%x.-|r|c%x%x%x%x%x%x%x%x%])|h", "%1")
  return msg
end
...
local function FillCopyWindow()
  ...
  if not Elephant.tempConf.is_copywindow_bbcode then
    ...
    local msg
    for i = Elephant.tempConf.currentline-Elephant.defaultConf.copywindowmaxlines, Elephant.tempConf.currentline do
      ...
      if msg and msg.type ~= "BN_WHISPER_INFORM" and msg.type ~= "BN_WHISPER" then
        msg = AddColorStrings(Elephant:GetLiteralMessage(msg, Elephant.db.profile.timestamps_in_copywindow))
        msg = CleanupMetaTypes(msg)
    ...
    for i = Elephant.tempConf.currentline-Elephant.defaultConf.copywindowmaxlines, Elephant.tempConf.currentline do
      ...
      if msg and msg.type ~= "BN_WHISPER_INFORM" and msg.type ~= "BN_WHISPER" then
        ...
        msg = string.gsub(msg, "|c%x%x(%x%x%x%x%x%x)|Hitem:(%d-):.-|h(.-)|h|r", "[url=" .. itemLinkSite .. "%2][color=#%1]%3[/color][/url]")
...
function Elephant:ShowCopyWindow()
  ...
  if not ElephantCopyFrame then
    CreateFrame("Frame", "ElephantCopyFrame", UIParent, "ElephantCopyFrameTemplate")
  end
...
function Elephant:ToggleBetweenNormalTextAndBBCode()
  if not Elephant.tempConf.is_copywindow_bbcode then
    Elephant.tempConf.is_copywindow_bbcode = true
  else
    Elephant.tempConf.is_copywindow_bbcode = false
  end
